...
if (!window.scriptLoaded) { //If scriptLoaded is not true (or not defined), run this block and set scriptLoaded to true.
	...
	document.addEventListener('DOMContentLoaded', function() {
		...
		//handleResourceErrors(); // Initialize resource error handling (Using inline solution due to ramdom fails)
		//handleImageErrors(); // Initialize image error handling (Using inline solution due to ramdom fails)

		/*
		var pages = [
			{ name: 'Home', url: 'index.htm' },
	...
	function handleResourceErrors() { // Ensure valid resource paths for index.html
		const resources = document.querySelectorAll('img, script, link[rel="stylesheet"]');

		resources.forEach(resource => {
			resource.onerror = function() {
				this.onerror = null;
				const fileName = this.getAttribute('src') || this.getAttribute('href');
				if (fileName) {
					const newFileName = fileName.split('/').pop();
					if (this.tagName.toLowerCase() === 'img') {
						this.src = newFileName;
					} else if (this.tagName.toLowerCase() === 'script' || this.tagName.toLowerCase() === 'link') {
						if (this.tagName.toLowerCase() === 'script') {
							this.src = newFileName;
						} else {
							this.href = newFileName;
						}
					}
				}
			};
	...
	function handleImageErrors() { // Ensure valid image paths for index.html (using handleResourceErrors instead)
		const images = document.querySelectorAll('img');

		images.forEach(img => {
			img.onerror = function() {
				this.onerror = null;
				const fileName = this.getAttribute('src').split('/').pop();
				this.src = fileName;
			};
		});
	}
